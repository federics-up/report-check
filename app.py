...
# ==============================================================================
    ...
    st.markdown("Carica i file del Calcio (Serie A / Estero / DAZN). Il controllo dei duplicati specchio è disattivato per questo formato.")
    ...
    if file_caricato is not None:
        try:
            ...
            # Controllo colonne obbligatorie calcio (meno rigido, adatta le tronche)
            campi_obbligatori = ["Emittente", "Brand", "Detections_MxM_Id", "Audience_AMR"]
            colonne_mancanti = [c for c in campi_obbligatori if c not in colonne_presenti]
            
            if colonne_mancanti:
                st.error("❌ Struttura Calcio non riconosciuta. Verificare la presenza dei campi portanti.")
                st.stop()
                
            righe_con_vuoti = df[campi_obbligatori].isnull().any(axis=1)
            
            tab_ver, tab_esp, tab_met = st.tabs(["🔍 Verifica Dati", "👀 Esplora e Filtra", "📈 Analisi Audience"])
            
            with tab_ver:
                st.subheader("Rapporto Qualità Calcio")
                st.success("✅ Didascalie e colonne del flusso calcio uniformate correttamente.")
                tot_vuoti = df[campi_obbligatori].isnull().sum().sum()
                if tot_vuoti > 0:
                    st.warning(f"⚠️ Rilevate {tot_vuoti} celle vuote nei campi chiave del calcio.")
                else:
